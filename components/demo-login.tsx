  ...
  return (
    <PageShell>
      <section className="relative isolate overflow-hidden bg-[#f4f8f6] px-4 py-10 sm:px-6 sm:py-14 lg:px-8 lg:py-20">
        ...
        <div className="login-card mx-auto grid w-full max-w-md overflow-hidden rounded-[28px] border border-[#d8e5e0] bg-white shadow-[0_28px_80px_rgba(21,41,35,0.14)] lg:max-w-5xl lg:grid-cols-[0.92fr_1.08fr]">
          <div className="login-intro relative overflow-hidden bg-[#123d34] px-7 py-10 text-white sm:px-10 sm:py-12 lg:px-12 lg:py-16">
          ...
          <div className="login-form px-7 py-10 sm:px-10 sm:py-12 lg:px-14 lg:py-16">
              ...
              {stage === "mobile" ? (
                <label className="block text-sm font-extrabold text-[#253831]">
                  ...
                  <div className="mt-2.5 flex h-14 items-center gap-3 rounded-xl border border-[#cbdad4] bg-white px-4 shadow-sm transition focus-within:border-[#167c74] focus-within:ring-4 focus-within:ring-[#167c74]/10">
                    ...
                    <input className="h-full min-w-0 flex-1 border-0 bg-transparent text-base font-semibold tracking-[0.08em] text-[#152923] outline-none placeholder:text-[#9aa8a3]" aria-label="Mobile number" value={mobile} onChange={(e) => setMobile(e.target.value.replace(/\D/g, "").slice(0, 10))} inputMode="numeric" placeholder="9999999999" autoFocus />
                  </div>
                </label>
              ) : (
                <label className="block text-sm font-extrabold text-[#253831]">
                  ...
                  <div className="mt-2.5 flex h-14 items-center gap-3 rounded-xl border border-[#cbdad4] bg-white px-4 shadow-sm transition focus-within:border-[#167c74] focus-within:ring-4 focus-within:ring-[#167c74]/10">
                    ...
                    <input className="h-full min-w-0 flex-1 border-0 bg-transparent text-base font-semibold tracking-[0.2em] text-[#152923] outline-none placeholder:text-[#9aa8a3]" aria-label="6-digit demo OTP" value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, "").slice(0, 6))} inputMode="numeric" placeholder="123456" autoFocus />
                  </div>
                  <small className="mt-2.5 block text-xs font-medium text-[#697a74]">Mock OTP: <strong>123456</strong></small>
                </label>
              )}
            </div>

            {error && <p className="mt-3 rounded-lg border border-red-200 bg-red-50 px-3.5 py-2.5 text-xs font-bold text-red-700" role="alert">{error}</p>}

            <div className="mt-6 grid gap-3">
              <button type="button" className="inline-flex h-12 w-full items-center justify-center rounded-xl bg-[#126f65] px-5 text-sm font-extrabold text-white shadow-[0_10px_24px_rgba(18,111,101,0.20)] transition hover:-translate-y-0.5 hover:bg-[#0f6259]" onClick={continueLogin}>{stage === "mobile" ? "Continue" : "Sign in to demo"}</button>
              <button type="button" className="inline-flex h-12 w-full items-center justify-center gap-3 rounded-xl border border-[#cbdad4] bg-white px-5 text-sm font-extrabold text-[#24372f] shadow-sm transition hover:-translate-y-0.5 hover:border-[#9fbeb4] hover:bg-[#f7faf9] disabled:cursor-wait disabled:opacity-60" onClick={continueWithGoogle} disabled={googleLoading}><span aria-hidden="true" className="grid h-6 w-6 place-items-center rounded-full border border-[#dce5e1] text-xs font-black text-[#4285f4]">G</span>{googleLoading ? "Opening Google…" : "Continue with Google"}</button>
            </div>

            {googleError && <p className="mt-3 rounded-lg border border-red-200 bg-red-50 px-3.5 py-2.5 text-xs font-bold leading-5 text-red-700" role="alert">{googleError}</p>}

            <button type="button" className="mt-5 w-full rounded-lg px-3 py-2 text-center text-xs font-extrabold text-[#126f65] underline decoration-[#9cc9bf] underline-offset-4 transition hover:bg-[#edf7f4]" onClick={fillDemo}>Fill demo login</button>
